...
class TreeNBTHandler( AbstractNBTHandler ):
    """
    NBT Event Handler that builds a tree of objects (similiar to a DOM) as they are fed to it.
    Call the getTree() function to return the tree after it is built.
    """
    def __init__( self ):
        self._t  = None          #Root of the tree
        self._n  = None          #Current name for next entry
        self._b  = None          #Current buffer (for TAG_Int_Array / TAG_Byte_Array)
        self._cs = deque()       #Containers that the current container is inside of
        self._c  = None          #The current container (for TAG_Compound and TAG_List)
        self._p  = self._putRoot #This function is called to put new elements into the current container. By default, the first element becomes the root of the tree.

    def getTree( self ):
        """Returns the tree of objects the handler built."""
        return self._t

    def _putRoot( self, value ):
        self._t = ( self._n, value )

    def _putList( self, value ):
        self._c.append( value )

    def _putDict( self, value ):
        name = self._n
        if name in self._c:
            raise NBTFormatError( "TAG_Compound already has an entry by name \"{}\"!".format( name ) )
        self._c[ name ] = value

    def _push( self, container, put ):
        #Push current container / put method to the stack
        self._cs.append( ( self._c, self._p ) )

        #Replace current container / put method with the given arguments
        self._c = container
        self._p = put

    def _pop( self ):
        #Replace current container / put method with previous ones from the stack
        self._c,self._p = self._cs.pop()
    ...
    def startList( self, tagType, length ):
        l = []
        self._p( l )
        self._push( l, self._putList )
    def endList( self ):
        self._pop()
    def startCompound( self ):
        t = OrderedDict()
        self._p( t )
        self._push( t, self._putDict )
    def endCompound( self ):
        self._pop()
    def startIntArray( self, length ):
        a = array( "l" )
